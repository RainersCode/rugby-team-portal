            ...
            <div className="space-y-4">
              {aboutData.team_highlights?.map((highlight: TeamHighlight, index: number) => (
                <motion.div 
                  ...
                  className="bg-white dark:bg-gray-800 rounded-none p-6 shadow-sm hover:shadow-md transition-shadow border-2 border-rugby-teal/20"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {teamMembers.map((member) => (
              <div key={member.id} className="text-center">
                <div className="relative w-48 h-48 mx-auto mb-4 overflow-hidden border-2 border-rugby-teal/20">
                  <Image
                    src={member.image}
                    alt={member.name}
                    fill
